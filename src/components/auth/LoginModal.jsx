...
const InputStyle = {
    ...
    background: '#F6F1E6', border: '1px solid #E6DAC0', borderRadius: 4,
    padding: '13px 16px 13px 44px', color: '#211A16', fontSize: '0.88rem',
...
function Field({ icon: Icon, ...props }) {
    return (
        <div style={{ position: 'relative', marginBottom: 16 }}>
            <Icon size={15} style={{ position: 'absolute', left: 15, top: '50%', transform: 'translateY(-50%)', color: '#B68D40' }} />
            <input style={InputStyle} {...props}
                onFocus={e => e.target.style.borderColor = '#B68D40'}
                onBlur={e => e.target.style.borderColor = '#E6DAC0'} />
        </div>
    );
}
    ...
    return (
        <>
            ...
            <div style={{
                ...
                zIndex: 301, background: '#F6F1E6', borderRadius: 4, width: '100%', maxWidth: 420,
                padding: '40px 36px', fontFamily: sans, boxShadow: '0 32px 80px rgba(26,14,9,0.35)',
            }}>
                <button onClick={onClose} style={{ position: 'absolute', top: 16, right: 16, background: 'none', border: 'none', cursor: 'pointer', color: '#8a7355' }}>
                    <X size={20} />
                </button>

                <div style={{ textAlign: 'center', marginBottom: 28 }}>
                    <h2 style={{ fontFamily: serif, fontSize: '1.8rem', color: '#211A16', margin: '0 0 6px', fontWeight: 600 }}>
                        {mode === 'login' ? 'Welcome Back' : 'Create Account'}
                    </h2>
                    <p style={{ color: '#8a7355', fontSize: '0.82rem', margin: 0 }}>
                ...
                <button onClick={handleGoogle} style={{
                    width: '100%', background: '#fff', border: '1px solid #E6DAC0', borderRadius: 4,
                    padding: '13px', fontSize: '0.85rem', fontWeight: 600, color: '#211A16',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
                    <hr style={{ flex: 1, border: 'none', borderTop: '1px solid #E6DAC0' }} />
                    <span style={{ fontSize: '0.72rem', color: '#8a7355', letterSpacing: '0.08em' }}>OR</span>
                    <hr style={{ flex: 1, border: 'none', borderTop: '1px solid #E6DAC0' }} />
                </div>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <button type="submit" disabled={loading} style={{
                        width: '100%', background: '#B68D40', color: '#211A16', border: 'none', borderRadius: 4,
                ...
                <p style={{ textAlign: 'center', fontSize: '0.8rem', color: '#5b4f45', margin: 0 }}>
                    {mode === 'login' ? "Don't have an account? " : 'Already have an account? '}
                    <button onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setError(''); }}
                        style={{ background: 'none', border: 'none', color: '#B68D40', fontWeight: 700, cursor: 'pointer', fontFamily: sans }}>
